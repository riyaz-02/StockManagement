...
class _InvoiceScreenState extends State<InvoiceScreen> {
  ...
  @override
  void dispose() {
    _custNameCtrl.dispose();   _custMobileCtrl.dispose();
    _custAddressCtrl.dispose(); _custPanCtrl.dispose();
    _goldRateCtrl.dispose();   _silverRateCtrl.dispose();
    _addlCtrl.dispose();       _discountCtrl.dispose();
  ...
  Future<void> _save() async {
    if (_vm.customerName.trim().isEmpty) {
      _snack('Please enter customer name', error: true); return;
    }
    if (_vm.tdsApplicable && _vm.customerPan.isEmpty) {
      _snack('Customer PAN is required (bill > ₹2,00,000)', error: true); return;
    }
  ...
  AppBar _buildAppBar() => AppBar(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
            _vm.invoiceNumber.isEmpty ? 'Generating number…' : _vm.invoiceNumber,
      ...
      child: Row(children: [
        Container(
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12)),
          child: const Icon(Icons.store_rounded, color: Color(0xFFF59E0B), size: 26),
        ),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
              Text('Invoice Date', style: TextStyle(fontSize: 9, color: Colors.grey[500])),
  ...
  Future<void> _pickDate({bool delivery = false}) async {
    ...
    setState(() {
      if (delivery) _vm.deliveryDate = picked;
      else _vm.invoiceDate = picked;
    });
    _vm.recalcTotals();
  }
  ...
  Widget _step1Customer(InvoiceViewModel vm) {
    return _card(child: Column(
      ...
      children: [
        ...
        _field('Customer Name *', _custNameCtrl,
        ...
        Row(children: [
          Expanded(child: _field('Mobile Number', _custMobileCtrl,
          ...
          Expanded(child: _field('Address', _custAddressCtrl,
            hint: 'City / Area',
          ...
          child: vm.tdsCardVisible
              ? Padding(
                  ...
                  child: Column(children: [
                    ...
                    const SizedBox(height: 10),
                    _field('Customer PAN Number *', _custPanCtrl,
                      maxLen: 10,
                      hint: 'e.g. ABCDE1234F',
    ...
    return _card(child: Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: [
            'West Bengal', 'Delhi', 'Maharashtra', 'Uttar Pradesh',
            'Rajasthan', 'Gujarat', 'Karnataka', 'Tamil Nadu',
            'Madhya Pradesh', 'Bihar', 'Other State',
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: vm.transactionType == 'intra-state'
                ? Colors.indigo.withAlpha(10) : Colors.deepPurple.withAlpha(10),
            borderRadius: BorderRadius.circular(10),
            border: Border.all(
              color: vm.transactionType == 'intra-state'
                  ? Colors.indigo.withAlpha(50) : Colors.deepPurple.withAlpha(50),
            ),
          ),
          child: Row(children: [
            Icon(
              vm.transactionType == 'intra-state'
                  ? Icons.home_work_outlined : Icons.flight_takeoff_outlined,
              size: 18,
              color: vm.transactionType == 'intra-state' ? Colors.indigo : Colors.deepPurple,
            ),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
              ...
              Text(
                ...
                style: TextStyle(
                  fontSize: 11,
                  color: vm.transactionType == 'intra-state'
                      ? Colors.indigo[300] : Colors.deepPurple[300],
                ),
              ),
            ])),
            Container(
  ...
  Widget _metalChip(InvoiceViewModel vm, int i, String type, String label) {
    ...
    return GestureDetector(
      onTap: () { vm.setMetalType(i, type); setState(() {}); },
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: item.taxableOverridden
              ? Colors.amber.withAlpha(15) : Colors.white,
          borderRadius: BorderRadius.circular(10),
          border: Border.all(
            color: item.taxableOverridden ? Colors.amber : const Color(0xFFE0E3EA),
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('Enter exact taxable amount as per bill.\n'
        ...
        actions: [
          if (item.taxableOverridden)
            TextButton(
              ...
              child: const Text('Reset to Auto', style: TextStyle(color: Colors.grey)),
            ),
      ...
      children: [
        ...
        TextFormField(
          ...
          onChanged: (v) {
            ...
            if (!vm.totalPayableOverridden) _paidCtrl.text = _fmt(vm.totalPayable);
          },
          decoration: InputDecoration(
            labelText: 'Discount on Making Charge',
            hintText: 'Reduces taxable amount (before GST)',
            suffixText: '₹',
            prefixIcon: const Icon(Icons.remove_circle_outline, color: Colors.redAccent, size: 18),
            filled: true,
        ...
        // GST lines
        if (isInter)
          _summaryRow('IGST @3%', _rupee(vm.totalIgst), color: Colors.deepPurple)
        else ...[ 
        ...
        if (vm.roundOff.abs() > 0.001) ...[ 
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(children: [
                const Icon(Icons.touch_app_outlined, color: Colors.teal, size: 18),
                const SizedBox(width: 8),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Tap to make round figure',
                      style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
                          color: Colors.teal)),
                  Text('Round-off: ${vm.roundOff >= 0 ? '+' : ''}${_fmt(vm.roundOff)}',
                      style: TextStyle(fontSize: 10, color: Colors.teal[300])),
                ])),
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: vm.totalPayableOverridden
                    ? Colors.amber : AppColors.primary.withAlpha(50),
        ...
        const SizedBox(height: 4),
        Center(child: Text('Double-tap to manually change total',
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          TextField(
            ...
            autofocus: true,
            decoration: const InputDecoration(
              labelText: 'Total Payable ₹', prefixText: '₹ ',
              border: OutlineInputBorder(),
            ),
            onTap: () => ctrl.selection =
                TextSelection(baseOffset: 0, extentOffset: ctrl.text.length),
        ...
        actions: [
          if (vm.totalPayableOverridden)
            TextButton(
              ...
              child: const Text('Reset Auto', style: TextStyle(color: Colors.grey)),
            ),
  ...
  Widget _step5Payment(InvoiceViewModel vm) {
    ...
    final badgeColor = isDue ? Colors.red : isAdvance ? Colors.green : Colors.teal;
    final badgeLabel = isDue ? 'Amount Due from Customer' : isAdvance ? 'Advance Paid' : 'Fully Settled ✓';
    ...
    return _card(child: Column(
      ...
      children: [
        ...
        _numericInput(
          ...
          ctrl: _paidCtrl,
          suffix: '₹',
          onChange: (v) { vm.setPaidAmount(double.tryParse(v) ?? 0); setState(() {}); },
        ),
        const SizedBox(height: 12),
        ...
        Container(
          ...
          child: Row(children: [
            Icon(isDue ? Icons.warning_amber_rounded
                : isAdvance ? Icons.check_circle_outline : Icons.check_circle_outline,
        ...
        Wrap(spacing: 8, runSpacing: 8,
            children: PaymentMode.values.map((m) {
              ...
              return GestureDetector(
                onTap: () { vm.setPaymentMode(m); setState(() {}); },
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 130),
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: sel ? AppColors.primary : Colors.grey.shade300),
                    boxShadow: sel ? [BoxShadow(
                        color: AppColors.primary.withAlpha(30), blurRadius: 6)] : null,
                  ),
  ...
  Widget _bottomBar(InvoiceViewModel vm) {
    final s  = vm.status;
    final sl = s == InvoiceStatus.pending ? 'Pending'
        : s == InvoiceStatus.active ? 'Active' : 'Delivered';
    final sc = s == InvoiceStatus.pending ? Colors.amber
        : s == InvoiceStatus.active ? Colors.blue : Colors.green;
